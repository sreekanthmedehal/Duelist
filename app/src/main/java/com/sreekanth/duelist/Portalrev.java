...
public class Portalrev extends Activity implements View.OnClickListener {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		myWebView = (WebView) findViewById(R.id.webview);
		   Intent iin= getIntent();
	    	 ...
	    	 final DataViewerSQLiteHelper stockSQLHelper = new DataViewerSQLiteHelper(Portalrev.this);
	...
	//	 myWebView.setVisibility(View.INVISIBLE);
		...
		myWebChromeClient = new MyWebChromeClient(Portalrev.this, childLayout, browserLayout, titleText);
			...
			public void onPageFinished(WebView view, String url) {
				...
				Log.v(Constants.LOG_TAG, "Finished: " + url);
				 if (url.contains("AgtLapsedPolicyDetails")) {
			    		//		 mHandler.post(new Runnable() {
			    		//             public void run() {
		//	    		    String polno = "655832749";        
//			      		 			
					 new Thread(new Task()).start();

		view.loadUrl("javascript: {" +
		    		  "document.getElementById(\"Agent_laps_policy_portletNetui_Form_4\").getElementsByTagName(\"input\")[0].value = "+ j2 +";" +
	
					  "document.getElementById(\"Agent_laps_policy_portletNetui_Form_4\").submit();" +
			          " " +	"};");
				
					
			          /*		 view.loadUrl("javascript: {" +
					"var tests = 	 document.getElementsByClassName(\"pagingLabel\")[1].innerHTML;" +
					"document.getElementById(\"Agent_laps_policy_portlet{actionForm.goToPage}\").value = tests;" +					 
					"document.getElementById(\"Agent_laps_policy_portletgo\").click();" + 
 				    " " +	"};");	 */
				 }
		 else if (url.contains("FcalcRevivalAmount")) {
	...
	class JIFace {
		...
		//	 System.out.println(data);
			 //DO the stuff
			 int position = data.indexOf(j2); 
 System.out.println(j2 + "------------------------------------------------------->" + String.valueOf(position));
 String subdata = (data.substring(position-91,position+1300));
 subdata = "<table>"+subdata +"</table>";
 myWebView.loadData(subdata, "text/html", "UTF-8");
		}
	}
